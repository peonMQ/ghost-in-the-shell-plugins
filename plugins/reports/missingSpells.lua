...
---@param name string
---@return spell|nil
local function getRankedSpell(name)
...
local function haveSpellOnly(name)
  ...
  if ranked and mq.TLO.Me.Book(ranked)() ~= nil then
    return true
  end
...
local function reportSpellStatus(expac, spell)
  local spellData = getRankedSpell(spell)
  if spellData == nil then
    logger.Error("UNLIKELY: %s: DID NOT RESOLVE [+r+]%s[+x+]", expac, spell)
  elseif not haveSpellOnly(spell) and not mq.TLO.Me.CombatAbility(spell)() ~= nil then
...
local function executeByExpansion(onlyExpac)
  ...
  local spells = classSpells[class]
  if spells == nil then
    logger.Error("UNLIKELY: class spells missing for %s", class)
    return
  end

  if onlyExpac == nil then
    logger.Info("Missing spell report")
  else
    logger.Info("Missing spell report (expansion <%s>):", onlyExpac)
  end
  ...
  for expac, expacSpells in pairs(spells) do
    if onlyExpac == nil or (onlyExpac ~= nil and string.lower(onlyExpac) == string.lower(expac)) then
      logger.Info("Expansion %s (%d total):", expac, #expacSpells)
  ...
  for i = 1, bookSize do
    ...
    if spell() ~= nil then
      if not recognizedSpell(spell) then
        logger.Info("UNRECOGNIZED SPELL %d: L%d %s", i, spell.Level(), spell.RankName())
      end
    end
  end
  ...
  for i = 1, 100 do
    ...
    if spell() ~= nil then
      if not recognizedSpell(spell) then
        logger.Info("UNRECOGNIZED DISC %d: L%d %s", i, spell.Level(), spell.RankName())
      end
    end
  end
end
...
local function create(commandQueue)
  ...
  binder.Bind("/fmspells", createCommand,
    "Tells all bots to report their missing spells ('expansion' optional if you want to only check the given expansion).",
    'expansion')
end
