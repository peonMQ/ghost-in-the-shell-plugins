...
local function executeById(id)
    ...
    if id < minValidSlotId or id > maxValidSlotId then
        ...
        broadcast.SuccessAll("Slot[%s] : %s", broadcast.ColorWrap(indexedSlotNames[id+1], 'Orange'), broadcast.ColorWrap(mq.TLO.Me.Inventory(id)() or '', 'Green'))
    end
    logger.Info("End [FIS]")
  end
  ...
  if validSlotNamesList[name] then
    broadcast.SuccessAll("Slot[%s] : %s", broadcast.ColorWrap(name, 'Orange'), broadcast.ColorWrap(mq.TLO.Me.Inventory(name)() or '', 'Green'))
  else
...
local function create(commandQueue)
    ...
    binder.Bind("/fis", createCommand, "Tells all bots to report the item they currently have for given id or name.", 'slotid|slotname')
end
