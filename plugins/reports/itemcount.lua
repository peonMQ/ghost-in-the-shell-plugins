...
local function executeById(itemId)
  ...
  broadcast.SuccessAll("Count for itemId %s : %s", broadcast.ColorWrap(itemId, 'Green'),
    broadcast.ColorWrap(itemCount, 'Maroon'))
...
local function executeByName(name)
  ...
  local itemCount = findItemInInventory(matchFunction) + findItemInBank(matchFunction)
  broadcast.SuccessAll("Count for name %s : %s", broadcast.ColorWrap(name, 'Green'),
    broadcast.ColorWrap(itemCount, 'Maroon'))
...
local function create(commandQueue)
  ...
  local function createCommand(query)
    if assist.IsOrchestrator() then
      bci.ExecuteAllCommand("/fic " .. query)
    end
